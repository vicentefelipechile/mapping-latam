...
function ENT:Initialize()
	...
	self:SetNWInt("amount", EML.Gas.Amount)
	self:SetNWInt("maxAmount", EML.Gas.Amount)
...
function ENT:OnTakeDamage(dmginfo)
	if (EML.Gas.ExplosionType == 2) then
	...
	elseif (EML.Gas.ExplosionType == 1) then
		if self.gasSound then
			self.gasSound:Stop()
		end	
		self:VisualEffect()
	elseif (EML.Gas.ExplosionType == 0) then		
		return false
	end
end
