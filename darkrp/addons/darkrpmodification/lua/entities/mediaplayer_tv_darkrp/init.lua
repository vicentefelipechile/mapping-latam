...
function ENT:Initialize()
	...
	self.fuelDepleteRate = BM2CONFIG.BaseFuelDepletionRateVip

	//Max amount of watt to output (in KW)
	self.maxPowerOut = BM2CONFIG.GeneratorPowerOutputVip //1000W
...
function ENT:StartTouch(ent)
	if ent:GetClass() == "bm2_fuel" or ent:GetClass() == "bm2_large_fuel" then
		if not ent.used then
			ent.used = true
			ent:Remove()
			self.fuel = math.Clamp(self.fuel + ent.fuelAmount, 0, 4000)
