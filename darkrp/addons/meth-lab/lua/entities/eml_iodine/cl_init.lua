...
function ENT:Draw()
	...
	local iodineColor = EML.Iodine.Color
	
	if (self:GetNWInt("amount")>0) then
		iodineColor = EML.Iodine.Color
	else
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML.DrawDistance then
		cam.Start3D2D(pos+ang:Up()*3.35, ang, 0.07)
			draw.SimpleTextOutlined("Yodo", "methFont", 0, -14, iodineColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100))
			draw.SimpleTextOutlined("Liquido", "methFont", 0, 10, iodineColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100))
	...
	ang:RotateAroundAxis(ang:Right(), 90)		
		cam.Start3D2D(pos+ang:Up()*3.35, ang, 0.1)
			...
			surface.SetDrawColor(EML.Iodine.Color)
